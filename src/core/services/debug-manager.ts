    ...
    #addCopyButton(): void {
        ...
        btn.on('click', () => {
            ...
            if (state.children && state.children instanceof Array) {
                ...
                const pointerPulseStrength =
                    pointer.children[3]['binding']['value']
                const pointerPulseFrequency =
                    pointer.children[4]['binding']['value']

                const modelPoint = state.children[2]
                const modelPointSize =
                    modelPoint.children[0]['binding']['value']
